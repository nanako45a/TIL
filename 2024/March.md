# 3/5
- PF開発：デバイスサイズに合わせたレイアウトの修正
- 就活準備：職務経歴書訂正
- 公的手続き：雇用保険説明会

# 3/6
- TILリポジトリ作成
- PF開発：デバイスサイズに合わせたレイアウトの修正、hotwire,turbo,ajax化の調査
- 公的手続き：初回講習・面談、国民年金の確認

# 3/7
- PF開発：ブックマーク機能のajax化（投稿一覧画面、投稿詳細画面）

# 3/8
- PF開発：ブックマーク機能のajax化
- 就活準備：自己分析

# 3/9
- PF開発：ブックマーク機能のajax化、投稿一覧画面のUI修正、オートコンプリート

# 3/10
- PF開発：ログアウト時のブックマーク機能の挙動修正、図書館データ登録、本番環境動作確認、全体レイアウト修正

# 3/11
- PF開発：ER図・README修正、レビュー提出
- 公的手続き：確定申告調べる
  
# 3/12
- PF開発：コメント機能のエラー対応
- 就活準備：面接質問集

# 3/13
- 就活準備：面接質問集

# 3/14
- PF開発：タスクのissue化、git管理下ファイルの整理、投稿編集画面ビューの崩れ修正

# 3/15
- 就活準備：就活サポート面談（企業軸、開発との進め方について）、面接トレーニング（通常①）
- PF開発：タスクのissue修正

# 3/16
- 就活準備：面接質問集、RUNTEQ企業研究

# 3/17
- PF開発：検索機能、オートコンプリート
- 就活準備：面接トレーニングレビュー提出（通常①）

# 3/18
- 就活準備：面接トレーニング（技術）
- 公的手続き：確定拠出年金について

# 3/19
- 公的手続き：雇用保険認定の申請、市民税・国民年金支払い

# 3/20
- PF開発：検索機能、オートコンプリート

# 3/21
- 就活準備：面接トレーニング（通常①）

# 3/22
- 就活準備：dodaセミナー、面接トレーニングレビュー再提出（通常①）
- 公的手続き：確定拠出年金

# 3/23
- PF開発：検索機能、オートコンプリート
- 就活準備：dodaカウンセリング、企業研究、面接回答集リファクタリング、環境調整
  
# 3/24-25
- なし

# 3/26
- 就活準備：模擬面接（通常1回目）、dodaセミナー
- PF開発：オートコンプリート、投稿一覧にturboframe利用

# 3/27
- 就活準備：doda面談、企業研究、面接回答集ブラッシュアップ（通常）
- PF開発：レコメンド機能の表示を地図へ変更

# 3/28
- 就活準備：面接回答集ブラッシュアップ（通常）

# 3/29
- なし

# 3/30
- PF開発：オートコンプリート

# 3/31
- PF開発：面接回答集ブラッシュアップ（通常）

# 4/1
- 就活準備：模擬面接（通常②）、面接トレーニング

# 4/2
- PF開発：オートコンプリート、レビュー提出

# 4/3
- 就活準備：dodaセミナー、企業研究、面接回答集（技術）

# 4/4
- 就活準備：面接回答集（技術）

# 4/5
- 就活準備：面接トレーニング

# 4/6
- 就活準備：自己分析

# 4/7
- 就活準備：就活面談

# 4/8
- PF開発：ファイルの整理

# 4/9
- 就活：A社面接

# 4/13
- 就活準備：面接回答集（技術）

# 4/14
- 就活準備：企業研究

# 4/15
- 就活：適性検査

# 4/16
- 就活：A社面接

# 4/17
- 就活準備：面接回答集（技術）

# 4/18
- 就活準備：面接回答集（技術）
- その他：病院

# 4/19
- 就活準備：面接回答集（技術）

# 4/21
- 就活準備：面接回答集（技術）

# 4/22
- 就活準備：面接回答集（技術）

# 4/23
- 学習：progate React、基本情報技術者（基礎理論）

# 4/24
- 就活準備：面接トレーニングLGTM（技術）

# 4/26
- 就活準備：模擬面接（技術）

# 4/27
- PF開発：図書館画像登録

# 4/28
- 就活準備：模擬面接（通常）、面接回答集ブラッシュアップ

# 4/29
- 就活準備：面接回答集ブラッシュアップ
- PF開発：新規投稿画面の位置情報登録機能修正、図書館情報登録

# 4/30
- 就活準備：面接回答集ブラッシュアップ
- PF開発：図書館情報登録
- その他：結婚式準備

# 5/1
- 就活準備：面接回答集ブラッシュアップ
- PF開発：図書館情報登録
- その他：消化器検診

# 5/3
- 就活準備：面接回答集ブラッシュアップ
- その他：親戚結婚式

# 5/6
- 就活準備：doda企業応募、面接回答集ブラッシュアップ

# 5/7
- 就活準備：wantedly企業研究、職務経歴書修正、面接回答集ブラッシュアップ

# 5/8
- PF開発：wantedly企業応募、位置情報登録機能修正

# 5/9
- PF開発：トップページにアプリの説明追加

# Todo
- 面接回答集を本多CAに就活チャンネルで提出4/30
- 通常模擬面接実施5/8
- トップページ修正
- 新規投稿画面の位置情報登録修正
- 頑張った機能の実装手順まとめる

# 毎日すること
- 面接回答集声だし練習毎日30分
- 図書館情報毎日3件以上登録
- PF毎日どこかは改善


# 4/27-5/6：GW